...
dump_default_args = default_args + [
    "--skip-comments",
    "--compact",
    "--disable-keys",
    "--no-create-db"
...
def import_schema():
    ...
    command(['mysql'] + default_args + [schema_db_name],
            input=file_content(current_schema_file),
            file=current_schema_file)
    cmd_to_file((['mysqldump', '--databases', schema_db_name] + dump_default_args),
                file=f'{schema_db_name}.dump.sql')
...
def import_updates():
    ...
    if not tag_list_str:
        ...
        print("::error::Tag list not formatted correctly!")
        exit(1)
    ...
    for version in versions:
        ...
        command(['mysql'] + default_args + [update_db_name],
                input=file_content(file),
                file=file)

    cmd_to_file((['mysqldump', '--databases', update_db_name] + dump_default_args),
                file=f'{update_db_name}.dump.sql')
...
command([
    'diff', '--side-by-side',
            ...
            '-W', '200',
            f'{update_db_name}.dump.sql', f'{schema_db_name}.dump.sql'
])
